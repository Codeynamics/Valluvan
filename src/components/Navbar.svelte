...
<nav class="sticky top-0 z-50 w-full border-b border-silver/15 bg-black/90 backdrop-blur">
  <div class="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
    <a href="/" use:link class="flex items-center gap-2 text-2xl font-bold text-gold">
    ...
    <div class="hidden items-center gap-8 md:flex">
      {#each links as l (l.name)}
        {#if l.isRoute}
          <a
            href={l.href}
            use:link
            class="text-sm font-medium text-white/60 transition-colors hover:text-gold"
          >
            {l.name}
          </a>
        {:else}
      ...
      <a
        href="/"
        use:link
        class="rounded-full bg-gold px-5 py-2.5 text-sm font-semibold text-black transition-colors hover:bg-gold-hover"
      >
        Get Started
      </a>
    </div>

    <button class="md:hidden" onclick={() => (menuOpen = !menuOpen)} aria-label="Toggle menu">
      <svg
    ...
    <div class="border-t border-silver/15 bg-black px-6 py-4 md:hidden">
      <div class="flex flex-col gap-4">
        {#each links as l (l.name)}
          {#if l.isRoute}
            <a
              href={l.href}
              use:link
              class="text-sm font-medium text-white/60 hover:text-gold"
        ...
        <a
          href="/"
          use:link
          class="rounded-full bg-gold px-5 py-2.5 text-center text-sm font-semibold text-black hover:bg-gold-hover"
        >
          Get Started
        </a>
      </div>
